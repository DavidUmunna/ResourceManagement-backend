...
function handleError(res, error, context) {
  ...
  if (status === 500) console.error(`driver.controller ${context}:`, error);
  return res.status(status).json({ success: false, message: error.message || "Server error" });
}

exports.create = async (req, res) => {
  try {
    const data = await service.createDriver(req.user, req.body);
...
exports.list = async (req, res) => {
  try {
    ...
    const data = await service.listDrivers(filter);
...
exports.update = async (req, res) => {
  try {
    const data = await service.updateDriver(req.user, req.params.id, req.body);
    return res.status(200).json({ success: true, data });
  } catch (e) {
